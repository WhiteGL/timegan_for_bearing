...
def tgan(dataX, parameters):
    ...
    def rnn_cell(module_name):
        ...
        # LSTM Layer Normalization
            ...
            rnn_cell = tf.compat.v1.layers.rnn.LayerNormBasicLSTMCell(num_units=hidden_dim, activation=tf.nn.tanh)
        return rnn_cell
    ...
    def recovery(H, T):
        ...
        with tf.compat.v1.variable_scope("recovery", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            X_tilde = tf.compat.v1.layers.dense(r_outputs, data_dim, activation=tf.nn.sigmoid)
    ...
    def generator(Z, T):
        ...
        with tf.compat.v1.variable_scope("generator", reuse=tf.compat.v1.AUTO_REUSE):
            e_cell = tf.keras.layers.StackedRNNCells([rnn_cell(module_name) for _ in range(num_layers)])
    ...
    G_loss_V1 = tf.reduce_mean(
        input_tensor=tf.abs(tf.sqrt(tf.nn.moments(x=X_hat, axes=[0])[1] + 1e-6) - tf.sqrt(tf.nn.moments(x=X, axes=[0])[1] + 1e-6)))
    ...
    timestamp = datetime.datetime.now().strftime("%d-%m-%y_%H_%M")

    saver = tf.compat.v1.train.Saver()
    #builder = tf.compat.v1.saved_model.Builder('./Model/')
    ...
    saver.save(sess, 'D:/python-project/timegan/model/'+timestamp)
